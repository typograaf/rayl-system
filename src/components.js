...
/* WHEN a label rolls, and it is not hover.
     press  it turns when the button is clicked. The default and very nearly the
            only answer: the movement means something happened, and crossing a
            control on the way to another one is not something happening.
     none   never on its own. The host calls turn() when its own state says so —
            an option group whose selection is decided elsewhere.
   A turn used to fire on pointerenter and on focus. In a panel the pointer
   crosses a dozen controls to reach one and every crossing turned a label, so
   the movement stopped meaning anything; the ground change already says which
   control is under the pointer. */
function upgradeButton(btn, opts){
  ...
  var when = (opts && opts.roll) || btn.dataset.roll || "press";
  if (when === "press") btn.addEventListener("click", function(){ r.turn(); });
...
function upgradeIconButton(btn){
  ...
  /* The body divides and the icon rides up on hover — neither is text. The
     label turns on the click, like every other label. */
...
function upgradeSelect(sel){
  ...
  OUTSIDE.push(function(t){ if (!sel.contains(t)) open(false); });
  return r;
}
/* Every open thing that has to shut when you click elsewhere shares one
   listener. A listener per instance is a listener per instance that nobody
   removes, and a page with ten selects on it was carrying ten. */
var OUTSIDE = [];
document.addEventListener("pointerdown", function(e){
  for (var i = 0; i < OUTSIDE.length; i++) OUTSIDE[i](e.target);
});
...
function upgradeTip(host){
  ...
  /* the node lives on the body so a control that clips its own contents cannot
     clip it, which means nothing else would ever take it away again */
  if (window.MutationObserver && host.parentNode){
    new MutationObserver(function(_, obs){
      if (!host.isConnected){ tip.remove(); obs.disconnect(); }
 ...
 * A panel where each control leans at the pointer as it passes is a panel of
 * things all asking to be clicked, which says nothing about which one you
 * want — the same reason a hover does not turn a label any more.
 *
